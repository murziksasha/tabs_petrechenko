...
function getTimeRemaining (endtime){  // создаём функцию которая будет высчитывать разницу во времени
  ...
  const days = Math.floor(t/(1000*60 *60 * 24)), //получение кол-ва дней
  //делим на 1000 что б получить секунды из миллисекунд
        hours = Math.floor((t/(1000*60*60)%24)), //получаем остаток кот-ый не влез в текущие сутки
        minutes = Math.floor((t/1000/60)%60),
        seconds = Math.floor((t/1000)%60);

    //для вывода значений функции наружу возвращаем объект
    return{
      'total':t, //общее кол-во миллисекунд и присваем переменной t, будет нажна для проверки не закончился ли таймер
      'days':days,
      'hours':hours,
      'minutes':minutes,
      'seconds':seconds
...
function getZero(num){
  if (num >= 0 && num < 10){ //проверка что б пришло число и оно меньше 10
      return `0${num}`;
  } else{
...
function setClock(selector, endtime){ //установка наших часов
  const timer = document.querySelector(selector), //установка таймера
        ...
        function udpateClock(){   //ф-я обновляет наш таймер каждую секунду
          ...
          //условия для того чтобы остановить таймер при достижении дефдлайна
          if (t.total <= 0){ //еесли время вышло то не обновляем наш таймер
...
//modal
  const modalTrigger = document.querySelectorAll('[data-modal]'), //кнопки вызывающие модальное окно
      modal = document.querySelector('.modal' ), // непослердсетенно само мод окно
  ...
  function openModal (){
    ...
    //закрытие модального окна при нажатии на любую точку сайта кнопку ESC
    ...
    document.addEventListener('keydown',(e)=>{
        if (e.code === 'Escape' && modal.classList.contains('show')){  //проверяем четко что нажатая кн будет строка Эскейп
    ...
    function showModalByScroll(){ //показ окна мод когда скролим
      if(window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1){ //это значит что пользователь долистал до конца страницы минус 1 пиксель что б не было бага
    ...
    class MenuCard {
      ...
      changToUah(){ //метод для конвертации валют
          this.price = this.price* this.transfer;
      }

      render(){ //метод для формирования верстки на сайти динамически
          ...
          if (this.classes.length === 0) { //если не будет передано параметр и вернется пустой массив благодаря rest
            this.element = 'menu__item'; //присваиваем класс по умолчанию
            element.classList.add(this.element);
          }else { //иначе если передан все таки аргументы с классами - присваиваем их
